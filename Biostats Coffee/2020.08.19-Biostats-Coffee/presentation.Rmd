...
* Use an iterative version of the swapping algorithm from @Puccetti2017-zl.
...
2. Swapping condition for a fixed $i \in \{1, \dots, n\}$, and indices $1 \leq k_1 < k_2 \leq k$: $$ \small \langle x_{\sigma_i(k_1)}^i, \sum_{j \neq 1} x_{\sigma_j(k_1)}^j \rangle + \langle x_{\sigma_i(k_2)}^i, \sum_{j \neq 1} x_{\sigma_j(k_2)}^j \rangle < \langle x_{\sigma_i(k_2)}^i, \sum_{j \neq 1} x_{\sigma_j(k_1)}^j\rangle + \langle x_{\sigma_i(k_1)}^i, \sum_{j \neq 1} x_{\sigma_j(k_2)}^j\rangle $$
